...
public class DataChart {
    ...
    public DataChart (ArrayList<Double> data) {
        
        if (data == null)  {
            throw new IllegalArgumentException("donnees null");
        }
        CategoryAxis xAxis = new CategoryAxis();
        xAxis.setLabel("Releves");
        ...
        series.setName("Requete 1 ");
        for (int i = 0; i < data.size(); i++) {
            String label = "Releve "+i; 
            series.getData().add(new Data<>(label, data.get(i)));
        }
        lineChart.getData().add(series);
        lineChart.setCreateSymbols(false);
